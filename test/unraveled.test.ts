...
describe('Trie', () => {
  ...
  */
  ...
  it('should be able to handle a fair bit of data', () => {
    ...
    console.log(
      `[trie] in-memory trie memory consumption: ${sizeof(trie.trieRoot) / 1024 / 1024}MB`
    )
    startTime = new Date().getTime()
    trie.encode()
    const endcodeTime = new Date().getTime() - startTime
    console.log(
      `[trie] done encoding ${wordCount} word test, allocTime:${
        trie.allocationTime
      } totTime:${endcodeTime}ms, avg:${(endcodeTime / wordCount) * 1000}μs`
    )
    console.log(
